...
* **Earth-quake localization:** An earthquake described by a source term $q(x)=w(t)\delta(x-s)$ is recorded by multiple seimosgraphs at locations $\Delta = \{r_k\}_{k=1}^n$. The goal is to recover $q$ in order to determine the location of the earthquake.
